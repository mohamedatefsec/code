...
export default function QuizBuilderPage({
  ...
  // فلاتر بنك الأسئلة (اختيار درس واحد أو أكتر مع مستوى صعوبة وحالة النشر) -
  // بتتحكم في القائمة القابلة للتصفح جوّه "+ إضافة أسئلة من بنك المادة".
  ...
  useEffect(() => {
    if (!quiz?.subjectId) return;
    // من غير فلتر status هنا: الأدمن عايز يشوف كل الأسئلة (مسودة ومنشورة)
    // في قائمة الإضافة اليدوية، ويقرر بنفسه ينشر أي سؤال يختاره أو يسيبه
    // مسودة - عبر زرار toggleQuestionStatus تحت.
    const params = new URLSearchParams({ subjectId: quiz.subjectId });
  ...
  }, [quiz?.subjectId, filterLessonIds, filterDifficulty, filterStatus]);
  ...
  /// نشر سؤال أو إرجاعه لمسودة من داخل صفحة الاختبار مباشرة - سواء وهو
  /// لسه في قائمة بنك الأسئلة أو بعد ما اتضاف لـ"الأسئلة المختارة".
  /// بيحدّث questionsById وbankQuestions محليًا بعد نجاح الطلب، من غير
  /// ما يحتاج يعيد تحميل كل حاجة.
  async function toggleQuestionStatus(q: BankQuestion) {
    const nextStatus = q.status === "published" ? "draft" : "published";
    setTogglingStatusId(q.id);
    ...
    if (res.ok) {
      setQuestionsById((prev) => ({ ...prev, [q.id]: { ...prev[q.id], status: nextStatus } }));
      setBankQuestions((prev) => prev.map((x) => (x.id === q.id ? { ...x, status: nextStatus } : x)));
    } else {
      setError("تعذّر تغيير حالة السؤال.");
    }
  }
            ...
            {selectedQuestions.map((q, i) => (
              <div key={q.id} className="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm">
                ...
                <button
                  ...
                  disabled={togglingStatusId === q.id}
                  title="اضغط لتغيير حالة نشر السؤال"
                  className={`rounded-full px-2 py-0.5 text-xs font-medium shrink-0 disabled:opacity-60 ${
                    q.status === "published"
                      ? "border border-accent/40 bg-accent/10 text-accent"
                      : "border border-border bg-canvas text-ink-soft"
                  }`}
                >
                  {togglingStatusId === q.id ? "..." : q.status === "published" ? "منشور" : "مسودة"}
                </button>
        ...
        <details className="pt-2 border-t border-border">
          ...
          <div className="mt-3 space-y-3">
            ...
            <select
              ...
              className="rounded-lg border border-border px-3 py-1.5 text-xs"
            >
              <option value="">كل الحالات (مسودة ومنشور)</option>
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto">
              {bankQuestions.length === 0 && (
                <p className="text-sm text-ink-soft">لا توجد أسئلة مطابقة لهذه الفلترة.</p>
              )}
              {bankQuestions.map((q) => (
                <div key={q.id} className="flex items-center gap-2 text-sm px-1 py-1">
